  ...
  return (
    ...
    >
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-white">Meet your sidebar</h2>
        ...
        {features.map((item, i) => (
          ...
          >
            <div className="w-10 h-10 rounded-lg bg-white/[0.05] flex items-center justify-center shrink-0">
              <item.icon className="w-5 h-5 text-noska-blue" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white">{item.label}</p>
              <p className="text-xs text-[var(--text-secondary)]">{item.desc}</p>
            </div>
            <span className="text-[10px] font-mono text-[var(--text-secondary)] bg-white/[0.04] px-2 py-1 rounded-md border border-white/[0.06] shrink-0">
              {item.shortcut}
            </span>
          </motion.div>
        ))}
