...
export interface PromoTicketCardProps {
  id?: string;
  tag?: string; // "DISCOUNT"
  bonusValue?: string; // "32%", "12%", "25%"
  title?: string;
  description?: string;
  termsText?: string;
  promoCode?: string; // "NOSKA32"
  barcodeNumber?: string; // "1234567890"
  themeColor?: "purple" | "coral" | "cyan" | "dark";
  actionLabel?: string; // "REDEEM"
  actionUrl?: string;
  viewUrl?: string;
  isClaimed?: boolean;
  onClaim?: () => void;
...
export function PromoTicketCard({
  ...
  const handleClaim = (e: React.MouseEvent) => {
    ...
    setClaimed(true);
    onClaim?.();
    showNotification(`🎉 Promo Redeemed! Code "${promoCode}" copied.`);
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          <div className="absolute -top-2.5 -right-2 w-4 h-4 rounded-full bg-[var(--background,#ffffff)] dark:bg-[#0B0C0E] shadow-[inset_0_-1.5px_2px_rgba(0,0,0,0.12)] z-20 pointer-events-none" />

          {/* Bottom Perforation Cutout Notch */}
          <div className="absolute -bottom-2.5 -right-2 w-4 h-4 rounded-full bg-[var(--background,#ffffff)] dark:bg-[#0B0C0E] shadow-[inset_0_1.5px_2px_rgba(0,0,0,0.12)] z-20 pointer-events-none" />
        ...
        <div className="flex-1 p-2 pl-2.5 pr-2 flex flex-col justify-between z-10 min-w-0">
          ...
          {/* Bottom Row: Barcode + Animated Action Pill Button */}
