...
const APPLE_SVG = (
  <svg className="w-4 h-4 mr-3 shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
...
export default function ProviderButton({
  ...
  const icon = provider === "google" ? GOOGLE_SVG : provider === "microsoft" ? MICROSOFT_SVG : provider === "apple" ? APPLE_SVG : GITHUB_SVG;
  const label = provider === "google" ? "Continue with Google" : provider === "microsoft" ? "Continue with Microsoft" : provider === "apple" ? "Continue with Apple" : "Continue with GitHub";
