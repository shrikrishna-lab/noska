 ...
 * Type is "todo" so cards render with a checkbox on the source page. */
...
export function makeStudyBlock(
  ...
  // Todo blocks give cards a visible checkbox affordance on the source page
  // ("review me") while the answer stays hidden in `study` metadata and the
  // real flip/rate interaction lives in the Spaced Repetition view.
  const base = blockFor("todo", card.front) as unknown as StudyCardBlock;
  base.id = uid();
