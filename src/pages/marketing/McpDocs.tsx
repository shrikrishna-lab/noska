...
function Code({ children }: { children: string }) {
  ...
  return (
    <div className="relative group">
      <pre className="overflow-x-auto rounded-xl border border-[var(--border)] bg-[#0d1428] p-4 font-mono text-[12px] leading-relaxed text-[var(--text)]">
        {children}
      </pre>
      <button
        onClick={() => { navigator.clipboard?.writeText(children); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
        className="absolute right-3 top-3 flex items-center gap-1 rounded-lg bg-white/5 px-2 py-1 text-[10px] text-[var(--secondary)] opacity-0 transition group-hover:opacity-100 hover:bg-white/10"
      >
...
export default function McpDocs() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      {/* Hero */}
      <header className="border-b border-[var(--border)] bg-gradient-to-b from-[var(--panel)] to-[var(--bg)]">
                          ...
                          {g.tools.map((t) => (
                            <tr key={t.name} className="border-b border-[var(--border)]/40 last:border-0">
                              <td className="py-2 pr-3 align-top">
                                <code className="font-mono font-semibold text-[var(--noska-blue,#4cc2ff)]">{t.name}</code>
...
curl -X POST http://127.0.0.1:8000/ \\
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/list"}'`}</Code>
        ...
        <div className="rounded-2xl border border-[var(--accent)]/25 bg-gradient-to-br from-[var(--accent)]/[0.08] to-transparent p-8 text-center">
          ...
          <Link to="/login" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-[var(--accent)] px-6 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90">
