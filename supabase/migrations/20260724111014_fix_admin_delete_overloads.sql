-- Fix: drop the text-overload of admin_delete (kept from an older migration)
-- and ensure only the uuid-overload exists to resolve PostgREST ambiguity (PGRST203)

DROP FUNCTION IF EXISTS public.admin_delete(p_session_token text, p_table text, p_id text, p_min_role text);

-- Recreate the uuid version (in case the drop above also removed it)
