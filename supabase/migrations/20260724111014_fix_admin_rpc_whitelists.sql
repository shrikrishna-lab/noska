-- Fix admin RPC whitelists: several tables used by Platform pages went missing
-- when later migrations redefined without carrying forward the full list.
-- Also add missing tables to the realtime publication.

-- Enable realtime for tables that were not in the publication but have useRealtimeInvalidate subscriptions
alter publication supabase_realtime add table if not exists public.audit_events;
alter publication supabase_realtime add table if not exists public.ai_chats;
alter publication supabase_realtime add table if not exists public.roadmap_items;
alter publication supabase_realtime add table if not exists public.roadmap_votes;

-- ──────────────────────────────────────────
-- 1. admin_select — add missing Platform tables
--    Missing: changelog_entries, blog_posts, legal_pages, admin_broadcasts, roadmap_votes
-- ──────────────────────────────────────────
...
-- ──────────────────────────────────────────
-- 2. admin_insert — add missing tables
--    Missing: roadmap_items, admin_broadcasts, integrations,
...
-- ──────────────────────────────────────────
-- 3. admin_delete — add missing tables
--    Missing: roadmap_items, integrations, webhook_deliveries,
