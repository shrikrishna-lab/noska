-- ============================================================
-- Migration: Content Tables (changelog, blog, legal pages)
-- Creates tables + updates admin RPC allowed_tables
-- ============================================================
...
CREATE POLICY "changelog_entries_rpc_only" ON public.changelog_entries FOR ALL USING (false);
CREATE POLICY "anon_read_published_changelog" ON public.changelog_entries
  FOR SELECT USING (published = true AND auth.role() = 'anon');
...
CREATE TABLE IF NOT EXISTS public.blog_posts (
  ...
  excerpt text,
  content text,
  author text DEFAULT 'Noska Team',
  cover_image text,
  tags text[] DEFAULT '{}',
  published boolean DEFAULT false,
  published_at timestamptz,
...
CREATE POLICY "blog_posts_rpc_only" ON public.blog_posts FOR ALL USING (false);
CREATE POLICY "anon_read_published_blog" ON public.blog_posts
  FOR SELECT USING (published = true AND auth.role() = 'anon');

-- ============================================================
-- 3. Legal pages (privacy, terms, etc.)
-- ============================================================
...
CREATE POLICY "legal_pages_rpc_only" ON public.legal_pages FOR ALL USING (false);
CREATE POLICY "anon_read_published_legal" ON public.legal_pages
  FOR SELECT USING (published = true AND auth.role() = 'anon');

-- ============================================================
-- 4. Update admin_select allowed_tables
-- ============================================================
...
-- ============================================================
-- 5. Update admin_insert allowed_tables
-- ============================================================
...
-- ============================================================
-- 6. Update admin_update allowed_tables
-- ============================================================
...
-- ============================================================
-- 7. Update admin_delete allowed_tables
-- ============================================================
...
-- ============================================================
-- 8. Grant anon access to published content
-- ============================================================
