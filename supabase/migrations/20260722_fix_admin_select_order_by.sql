-- Fix admin_select: make ORDER BY conditional (skip when p_order_col IS NULL)
-- Tables like launch_settings and waitlist_settings only have updated_at, not created_at,
-- so a default of 'created_at' causes 42703/column-not-found errors.

CREATE OR REPLACE FUNCTION public.admin_select(
  p_session_token text,
  p_table text,
  p_select text DEFAULT '*'::text,
  p_order_col text DEFAULT NULL::text,
  p_order_dir text DEFAULT 'desc'::text,
  p_limit integer DEFAULT NULL::integer,
  p_eq_col text DEFAULT NULL::text,
  p_eq_val text DEFAULT NULL::text
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
